...
class Layers(object):
    ...
    def param_init_lstm_cond(self, options, params,
        ...
        Wc_att_c = norm_weight(dimctx_c, ortho=False)
        params[_p(prefix, 'Wc_att_c')] = Wc_att_c

        # attention: LSTM -> hidden
        Wd_att = norm_weight(dim, dimctx)
        params[_p(prefix, 'Wd_att')] = Wd_att
        Wd_att_c = norm_weight(dim, dimctx_c)
        params[_p(prefix, 'Wd_att_c')] = Wd_att_c

        # attention: hidden bias
        b_att = numpy.zeros((dimctx,)).astype('float32')
        params[_p(prefix, 'b_att')] = b_att
        b_att_c = numpy.zeros((dimctx_c,)).astype('float32')
        params[_p(prefix, 'b_att_c')] = b_att_c
        ...
        U_att_c = norm_weight(dimctx_c, 1)
        params[_p(prefix, 'U_att_c')] = U_att_c
        c_att_c = numpy.zeros((1,)).astype('float32')
        params[_p(prefix, 'c_tt_c')] = c_att_c

        if options['selector']:
            # attention: selector
            W_sel = norm_weight(dim, 3)
    ...
    def lstm_cond_layer(self, tparams, state_below, options, prefix='lstm',
                        mask=None, context=None, context_c=None, one_step=False,
        ...
        assert context_c, 'Context must be provided'
        ...
        pctx_c_ = tensor.dot(context_c, tparams[_p(prefix, 'Wc_att_c')]) + tparams[
            _p(prefix, 'b_att_c')]
        if one_step:
        ...
        Wd_att_c = tparams[_p(prefix, 'Wd_att_c')]
        U_att_c = tparams[_p(prefix, 'U_att_c')]
        c_att_c = tparams[_p(prefix, 'c_tt_c')]
        if options['selector']:
        ...
        def _step(m_, x_,  # sequences
                  h_, c_,  # outputs_info
                  pctx_, pctx_c_, context, context_c, Wd_att, Wd_att_c, U_att, U_att_c,
                  c_att, c_att_c, W_sel, b_sel, U,  # non_sequences
            ...
            pstate_c_ = tensor.dot(h, Wd_att_c)
            pctx_c_ = pctx_c_ + pstate_c_[:, None, :]
            pctx_c_ = tanh(pctx_c_)
            ...
            alpha_c = tensor.dot(pctx_c_, U_att_c) + c_att_c
            alpha_pre = alpha_c
            alpha_shp = alpha_c.shape
            alpha_c = tensor.nnet.softmax(alpha_c.reshape([alpha_shp[0], alpha_shp[1]]))  # softmax
            ctx_ = (context * alpha[:, :, None]).sum(1)  # (m, ctx_dim)
            ctx_c_ = (context_c * alpha_c[:, :, None]).sum(1)  # (m, ctx_dim)
            if options['selector']:
                weight = tensor.nnet.softmax(tensor.dot(h_, W_sel) + b_sel)
                weight_1 = weight[:,0]
                weight_1 = weight_1.reshape([weight_1.shape[0]])
                weight_2 = weight[:, 1]
                weight_2 = weight_2.reshape([weight_2.shape[0]])
                ctx_ = weight_1[:,None] * ctx_
                ctx_c_ = weight_2[:,None] * ctx_c_

            rval = [h, c, alpha, alpha_c, ctx_, ctx_c_, weight, pstate_, pctx_,pctx_c_,
                     i, f, o, preact, alpha_pre]
            return rval

        if options['use_dropout']:
            _step0 = lambda m_, x_, dp_, h_, c_, \
                            pctx_, pctx_c_, context, context_c, Wd_att, Wd_att_c,\
                            U_att, U_att_c, c_att, c_att_c, W_sel, b_sel, U: _step(
                m_, x_, h_, c_,
                pctx_, pctx_c_, context, context_c, Wd_att, Wd_att_c, U_att, U_att_c,
                c_att, c_att_c, W_sel, b_sel, U, dp_)
            ...
            _step0 = lambda m_, x_, h_, c_, \
                            pctx_, pctx_c_, context, context_c, Wd_att, Wd_att_c,\
                            U_att, U_att_c, c_att, c_att_c, W_sel, b_sel, U: _step(
                m_, x_, h_, c_, pctx_, pctx_c_, context, context_c,
                Wd_att, Wd_att_c, U_att, U_att_c, c_att, c_att_c, W_sel, b_sel, U)
        ...
        if one_step:
            if options['use_dropout']:
                rval = _step0(
                    ...
                    pctx_, pctx_c_, context, context_c, Wd_att, Wd_att_c, U_att, U_att_c,
                    c_att, c_att_c, W_sel, b_sel, U)
            else:
                rval = _step0(mask, state_below, init_state, init_memory,
                              pctx_, pctx_c_, context, context_c, Wd_att, Wd_att_c,
                              U_att, U_att_c, c_att, c_att_c, W_sel, b_sel, U)
        else:
            ...
            rval, updates = theano.scan(
                ...
                non_sequences=[pctx_, pctx_c_, context, context_c,
                               Wd_att, Wd_att_c, U_att, U_att_c, c_att, c_att_c, W_sel, b_sel, U],
                name=_p(prefix, '_layers'),
