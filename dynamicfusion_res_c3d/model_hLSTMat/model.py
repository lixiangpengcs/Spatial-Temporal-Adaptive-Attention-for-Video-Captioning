...
class Model(object):
    ...
    def init_params(self, options):
        ...
        params = self.layers.get_layer('ff')[0](
            options, params, prefix='ff_state_c', nin=ctx_dim_c, nout=options['dim'])
        params = self.layers.get_layer('ff')[0](
            options, params, prefix='ff_memory_c', nin=ctx_dim_c, nout=options['dim'])
        ...
        if options['ctx2out']:
            ...
            params = self.layers.get_layer('ff')[0](
                options, params, prefix='ff_logit_ctx_c',
                nin=ctx_dim_c, nout=options['dim_word'])
            params = self.layers.get_layer('ff')[0](
    ...
    def build_model(self, tparams, options):
        ...
        init_state_c = self.layers.get_layer('ff')[1](
            tparams, ctx_mean_c, options, prefix='ff_state_c', activ='tanh')
        init_memory_c = self.layers.get_layer('ff')[1](
            tparams, ctx_mean_c, options, prefix='ff_memory_c', activ='tanh')
        ...
        if options['ctx2out']:
            ...
            ctxs_beta_c = self.layers.get_layer('ff')[1](
                tparams, ctxs_c, options, prefix='ff_logit_ctx_c', activ='linear')
            to_lstm_h = self.layers.get_layer('ff')[1](
    ...
    def build_sampler(self, tparams, options, use_noise, trng, mode=None):
        ...
        bo_init_state_c = self.layers.get_layer('ff')[1](
            tparams, ctx_mean_c, options, prefix='ff_state_c', activ='tanh')
        bo_init_memory_c = self.layers.get_layer('ff')[1](
            tparams, ctx_mean_c, options, prefix='ff_memory_c', activ='tanh')
        ...
        f_init = theano.function(
            [ctx0, ctx_mask, ctx0_c, ctx_mask_c],
    ...
    def gen_sample(self, tparams, f_init, f_next, ctx0, ctx0_c, ctx_mask, ctx_mask_c, options,
        ...
        for ii in xrange(maxlen):
            ...
            # next_state: [matrix]
            # next_memory: [matrix]
            rval = f_next(*([next_w, ctx0, ctx_mask, ctx0_c, ctx_mask_c] + next_state + next_memory))
            next_p = rval[0]
            if restrict_voc:
    ...
    def sample_execute(self, engine, options, tparams, f_init, f_next, x, ctx, ctx_c, ctx_mask, ctx_mask_c, trng):
        stochastic = False
        for jj in xrange(numpy.minimum(10, x.shape[1])):
            sample, score, _, _ = self.gen_sample(tparams, f_init, f_next, ctx[jj], ctx_c[jj], ctx_mask[jj],ctx_mask_c[jj],
