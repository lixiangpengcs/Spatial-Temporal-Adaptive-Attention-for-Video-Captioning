...
def generate_sample_gpu_single_process_ensemble(
        model_type, model_archive, options, options_c, engine, model,
        f_init, f_init_c, f_next, f_next_c,
    ...
    def sample(whichset):
        samples = []
        ctxs, ctx_masks, ctxs_c, ctx_masks_c = engine.prepare_data_for_blue(whichset)
        for i, ctx, ctx_c, ctx_mask, ctx_mask_c in zip(range(len(ctxs)), ctxs,ctxs_c, ctx_masks, ctx_masks_c):
            print 'sampling %d/%d' % (i, len(ctxs))
            sample, score, _, _, _, _ = model.gen_sample_ensemble(
                None,None, f_init, f_init_c, f_next,f_next_c, ctx, ctx_c,
                ctx_mask, ctx_mask_c, options, options_c,None, beam, maxlen=MAXLEN)
...
def compute_score_ensemble(
        model_type, model_archive, options, options_c, engine, save_dir,
        beam, n_process,
        whichset='both', on_cpu=True,
        processes=None, queue=None, rqueue=None, shared_params=None,
        one_time=False, metric=None,
        f_init=None, f_init_c=None, f_next=None, f_next_c=None, model=None):
    assert metric != 'perplexity'
    if on_cpu:
        raise NotImplementedError()
    else:
        assert model is not None
        samples_valid, samples_test = generate_sample_gpu_single_process_ensemble(
            model_type, model_archive, options, options_c,
            engine, model, f_init, f_init_c, f_next, f_next_c,
            save_dir=save_dir,
            beam=beam,
            whichset=whichset)
