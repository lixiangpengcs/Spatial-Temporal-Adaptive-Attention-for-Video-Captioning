...
class Model(object):
    ...
    def gen_sample_ensemble(self, tparams,tparams_c, f_init,f_init_c, f_next, f_next_c,
                            ctx0, ctx0_c, ctx_mask, ctx_mask_c, options, options_c,
                   trng=None, k=1, maxlen=30, stochastic=False,
                   restrict_voc=False):
        '''
        ctx0: (26,1024)
        ...
        # [(26,1024),(512,),(512,)]
        #print 'ctx0:',ctx0.shape,'ctx_mask:',ctx_mask.shape
        #print 'ctx0_c:', ctx0_c.shape, 'ctx_mask_c:', ctx_mask_c.shape
        rval = f_init(ctx0, ctx_mask)
        #print 'rval:',len(rval)
        rval_c = f_init_c(ctx0_c,ctx_mask_c)
        ctx0 = rval[0]
        ctx0_c = rval_c[0]
        ...
        for lidx in xrange(n_layers_lstm):
            next_state.append(rval[1 + lidx])
            next_state[-1] = next_state[-1].reshape([live_k, next_state[-1].shape[0]])
            next_state_c.append(rval_c[1 + lidx])
            next_state_c[-1] = next_state_c[-1].reshape([live_k, next_state_c[-1].shape[0]])
        for lidx in xrange(n_layers_lstm):
            next_memory.append(rval[1 + n_layers_lstm + lidx])
            next_memory[-1] = next_memory[-1].reshape([live_k, next_memory[-1].shape[0]])
            next_memory_c.append(rval_c[1 + n_layers_lstm + lidx])
            next_memory_c[-1] = next_memory_c[-1].reshape([live_k, next_memory_c[-1].shape[0]])
        next_w = -1 * numpy.ones((1,)).astype('int64')
        # next_state: [(1,512)]
        # next_memory: [(1,512)]
        for ii in xrange(maxlen):
            ...
            # next_memory: [matrix]
            rval = f_next(*([next_w, ctx0, ctx_mask] + next_state + next_memory))
            rval_c = f_next_c(*([next_w, ctx0_c, ctx_mask_c] + next_state_c + next_memory_c))
            next_p = rval[0]
            next_p_c = rval_c[0]
            next_p = (next_p+next_p_c)/2
            if restrict_voc:
                raise NotImplementedError()
            next_w = rval[1]  # already argmax sorted
            next_state = []
            ...
            for lidx in xrange(n_layers_lstm):
                next_state.append(rval[2 + lidx])
                next_state_c.append(rval_c[2 + lidx])
            next_memory = []
            next_memory_c = []
            for lidx in xrange(n_layers_lstm):
                next_memory.append(rval[2 + n_layers_lstm + lidx])
                next_memory_c.append(rval_c[2 + n_layers_lstm + lidx])
            if stochastic:
                sample.append(next_w[0])  # take the most likely one
                sample_score += next_p[0, next_w[0]]
                if next_w[0] == 0:
                ...
                for lidx in xrange(n_layers_lstm):
                    new_hyp_states.append([])
                    new_hyp_states_c.append([])
                new_hyp_memories = []
                new_hyp_memories_c = []
                for lidx in xrange(n_layers_lstm):
                    new_hyp_memories.append([])
                    new_hyp_memories_c.append([])
                ...
                for idx, [ti, wi] in enumerate(zip(trans_indices, word_indices)):
                    ...
                    for lidx in xrange(n_layers_lstm):
                        new_hyp_states[lidx].append(copy.copy(next_state[lidx][ti]))
                        new_hyp_states_c[lidx].append(copy.copy(next_state_c[lidx][ti]))
                    for lidx in xrange(n_layers_lstm):
                        new_hyp_memories[lidx].append(copy.copy(next_memory[lidx][ti]))
                        new_hyp_memories_c[lidx].append(copy.copy(next_memory_c[lidx][ti]))
                ...
                for idx in xrange(len(new_hyp_samples)):
                    if new_hyp_samples[idx][-1] == 0:
                        ...
                        for lidx in xrange(n_layers_lstm):
                            hyp_states[lidx].append(new_hyp_states[lidx][idx])
                            hyp_states_c[lidx].append(new_hyp_states_c[lidx][idx])
                        for lidx in xrange(n_layers_lstm):
                            hyp_memories[lidx].append(new_hyp_memories[lidx][idx])
                            hyp_memories_c[lidx].append(new_hyp_memories_c[lidx][idx])
                hyp_scores = numpy.array(hyp_scores)
                live_k = new_live_k
                ...
                for lidx in xrange(n_layers_lstm):
                    next_state.append(numpy.array(hyp_states[lidx]))
                    next_state_c.append(numpy.array(hyp_states_c[lidx]))
                next_memory = []
                next_memory_c = []
                for lidx in xrange(n_layers_lstm):
                    next_memory.append(numpy.array(hyp_memories[lidx]))
                    next_memory_c.append(numpy.array(hyp_memories_c[lidx]))
        ...
        return sample, sample_score, next_state, next_memory, next_state_c, next_memory_c
    ...
    def pred_probs(self, engine, whichset, f_log_probs, verbose=True):
        ...
        for index in iterator:
            ...
            x, mask, ctx, ctx_mask, ctx_c, ctx_mask_c = prepare_data(engine, tag)
    ...
    def sample_execute_ensemble(self, engine, options,options_c,tparams,tparams_c,
                                f_init,f_init_c, f_next, f_next_c, x, ctx, ctx_c,
                                ctx_mask, ctx_mask_c, trng):
        stochastic = False
        for jj in xrange(numpy.minimum(10, x.shape[1])):
            sample, score, _, _, _, _ = self.gen_sample_ensemble(tparams,tparams_c, f_init, f_init_c,
                                                  f_next, f_next_c, ctx[jj],ctx_mask[jj],
                                                  ctx_c[jj], ctx_mask_c[jj],options, options_c,
